...
$(function(){
    ...
    $('#cliente_cep').live('keyup',function(e){
        ...
        var cep = $.trim($('#cliente_cep').val()).replace('-','');
        if(cep.length === 8){
            $('#cliente_cep').blur();
            ...
            function (data) {
                if(data != '-1'){
                    ...
                    // }else{
                    //     $('#cliente_rua').val('CEP �nico - informe o nome da rua');
                    //     $('#cliente_rua').focus();
                    //     $('#cliente_rua').select();
                    // }                    
                }
    ...
    //verifica se o nome est� incompleto
        ...
        $.post(url,{
            ...
            if(data == 2){
                $('html, body').animate({
                    ...
                    elm.addClass('invalid').parent().find('span').html('* CPF Inv�lido');
                    elm.focus();  
                    elm.val('');
                    valid = false;
                });                 
            ...
            if(data == 1){
                $('html, body').animate({
                    ...
                    elm.addClass('invalid').parent().find('span').html('* CPF j� cadastrado');
                    elm.focus();
                    elm.val('');
                    valid = false;                    
                });                 
        ...
        $.post(url,{
            ...
            if(data == 2){
                $('html, body').animate({
                    ...
                    elm.addClass('invalid').parent().find('span').html('* E-mail Inv�lido');
                    elm.focus();  
                    elm.val('');
                    valid = false;
                });                 
            ...
            if(data == 1){
                $('html, body').animate({
                    ...
                    elm.addClass('invalid').parent().find('span').html('* E-mail j� cadastrado');
                    elm.focus();
                    elm.val('');
                    valid = false;                    
                });                 
