        ...
        if ( $this->login != null )
        {
            ...
            if ( isset( $this->uri_segment[2] ) )
            {
                ...
                if ( $this->result() )
                {
                    ...
                    $this->preg( array( '/1/', '/2/', '/3/', '/4/', '/6/', '/7/' ), array( 'warning', 'info', 'success', 'success', 'warning', 'warning' ), 'pedstatus' );
                    $this->preg( $this->status_pat, $this->status_rep, 'pedido_status' );
            ...
            $this->assign( 'BtnPayShow', ( $this->pedido_status == 1 ) ? '' : 'hide'  );
            if ( in_array( "show", $this->uri_segment ) )
            {
                ...
                {
                    $this->assign( 'showPayWindow', 'abreModal("dep");' );
                    $this->assign( 'ModalPageInclude', 1 );
                }
            }
            //btnMessage
            if ( $this->pedido_pay_gw <> 4 )
            {
                $this->assign( 'BtnPayMessage', 'Clique aqui para efetuar o pagamento' );
            }
            //pagSeguro
            if ( $this->pedido_pay_gw == 1 && $this->pedido_status < 3 )
            {
                $this->assign( 'PayWindow', 'ligthPag();' );
		$this->assign('BtnPayHref', 'javascript:void(0);');
            }
            //payPal
            if ($this->pedido_pay_gw == 2 && $this->pedido_status < 3) {
                $this->assign('showPayWindow', 'abreModal("paypal");');
                $this->assign('ModalPayPalInclude', 'paypal');
                $this->assign('ModalDepInclude', '');
                $this->assign('ModalMBInclude', '');
            }
            //deposito
            $this->assign( 'BtnPayComprovante', 'hide' );
            if ( $this->pedido_pay_gw == 4 && $this->pedido_status < 3 && $this->pedido_comprovante == 0 )
            {
                $this->assign( 'PayWindow', 'abreModal("dep");' );
                $this->assign( 'ModalPageInclude', 1 );
                $this->assign( 'BtnPayMessage', 'Clique aqui para enviar comprovantes ou ver nossos dados bancários' );
                $this->assign( 'BtnPayComprovante', 'hide' );
            }
            elseif ( $this->pedido_pay_gw == 4 && $this->pedido_comprovante <> 0 )
            {
                $this->assign( 'PayWindow', 'abreModal("dep");' );
                $this->assign( 'ModalPageInclude', 1 );
                $this->assign( 'BtnPayMessage', 'Clique aqui para reenviar comprovantes' );
                $this->assign( 'BtnPayComprovante', '' );
	        $this->assign('BtnPayHref', 'javascript:void(0);'); 
            }
	    elseif ($this->pedido_pay_gw == 2) {
                $this->assign('showPayWindow', '');
                $this->assign('BtnPayHref', $this->pedido_pay_url);
                $paypal_time =  round ( ( strtotime(date('Y-m-d H:i:s')) - strtotime($this->pedido_update) ) / 60 );
                if ($paypal_time >= 15) {
                    $this->faturaPayPal();
                }
                $this->assign('BtnPayTarget', 'target="new"');
                $this->assign('PayWindow', "abrePayPal(\"$this->pedido_pay_url\");");
                $this->assign('BtnPayMessage', 'Clique aqui para pagar sua fatura PayPal');
                $this->assign('BtnPayComprovante', 'hide');
                $this->assign('ModalMBInclude', '');
                $this->assign('ModalDepInclude', '');
                $this->assign('ModalPayPalInclude', 'paypal');
                $this->assign('pay_entidade', '');
                $this->tpl->modal = 'mbanco';
            }
